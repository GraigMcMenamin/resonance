...
struct ArtistDetailView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                Button {
                    if let uri = SpotifyService.spotifyURI(type: "artist", id: artistId),
                       UIApplication.shared.canOpenURL(uri) {
                        UIApplication.shared.open(uri)
                    } else if let webURL = SpotifyService.spotifyWebURL(type: "artist", id: artistId) {
        ...
        .task {
            ...
            async let artistLoad: Void = loadArtistData()
    ...
    private func loadBuddyRatings() async {
        ...
        do {
            ...
            buddyRatings = try await firebaseService.fetchRatingsForItemByUsers(spotifyId: artistId, userIds: buddyIds)
        } catch {
